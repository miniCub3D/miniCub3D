...
int BFS_map(t_game *game, t_deque *deque)
{
	...
	while (deque->cnt != 0)
	{
		...
		while (++i <= 4)
		{
			...
			printf("%d %d / %c %d\n", event_pos[1], event_pos[0], game->map_info.map[event_pos[1]][event_pos[0]], game->map_info.map_visited[event_pos[1]][event_pos[0]]);
			if (game->map_info.map[event_pos[1]][event_pos[0]] == '0' && game->map_info.map_visited[event_pos[1]][event_pos[0]] == 0)
			{
				append_head(deque, make_new_node(event_pos[0], event_pos[1]));
				game->map_info.map_visited[event_pos[1]][event_pos[0]] = 1;
			}
			else if(game->map_info.map[event_pos[1]][event_pos[0]] == ' ' 
			|| (event_pos[1] < 0) || (game->map_info.height <= event_pos[1]) || (event_pos[0] < 0) || (game->map_info.width <= event_pos[0]))

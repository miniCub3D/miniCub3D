...
void	make_visited_map(t_game *game)
{
	...
	game->map_info.map_visited = \
		(int **)malloc(sizeof(int *) * (game->map_info.height));
	i = 0;
	while (i < game->map_info.height)
	{
		game->map_info.map_visited[i] = \
			(int *)malloc(sizeof(int) * (game->map_info.width));
		j = 0;
...
void	check_map_surrounded_wall(t_game *game)
{
	...
	while (y < game->map_info.height)
	{
		...
		{
			if (game->map_info.map_visited[y][x] == 0
